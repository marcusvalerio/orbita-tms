...
export function SimulationProvider({ children }: { children: React.ReactNode }) {
  ...
  const dispatch = useCallback((action: SimulationAction) => { let succeeded = true; setData((prev) => { if (!prev) return prev; try { return reduce(prev, action); } catch (err) { console.error("Falha ao processar ação da simulação:", { action, error: err }); succeeded = false; return prev; } }); pushToast(succeeded ? toastForAction(action) : "Não foi possível concluir a ação. A operação não foi alterada."); }, [pushToast]);
  ...
  const resetSimulation = useCallback(() => { clearPersistedState(); setData(generateEmptyOperation()); pushToast("Simulação reiniciada — operação restaurada para o estado inicial (zero)."); }, [pushToast]);
  const loadDemoScenario = useCallback(() => { setData(generateAtlasOperation()); pushToast("Cenário de demonstração carregado."); }, [pushToast]);
  ...
  return <SimulationContext.Provider value={{ data, isEmpty, createOrder, createLoad, createShipment, startShipment, createOccurrence, resolveOccurrence, completeDelivery, resetSimulation, loadDemoScenario, createPartnerCompany, regeneratePartnerCode, createSolicitation, convertSolicitationToOrder, toasts }}>{children}</SimulationContext.Provider>;
}
